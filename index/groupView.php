...
<?php
	...
	if(!empty($keyword)){
		$listSql = "SELECT t1.groupId,t1.deptId,t1.id AS itemId,t1.itemName,t1.itemLevel,t1.phones,t1.itemArea,t1.linkMan,t2.groupName,t2.groupLevel,t3.deptName FROM `bbs_item` t1 LEFT JOIN `bbs_group` t2 ON t1.groupId = t2.id LEFT JOIN `bbs_dept` t3 ON t1.deptId = t3.id WHERE groupId='$groupId' AND (t1.itemName LIKE '%$keyword%' OR t3.deptName LIKE '%$keyword%' OR t2.groupName LIKE '%$keyword%' OR t1.linkMan LIKE '%$keyword%' OR t1.phones LIKE '%$keyword%') ORDER BY itemLevel,itemId";				
	}else{
		$listSql = "SELECT t1.groupId,t1.deptId,t1.id AS itemId,t1.itemName,t1.itemLevel,t1.phones,t1.itemArea,t1.linkMan,t2.groupName,t2.groupLevel,t3.deptName FROM `bbs_item` t1 LEFT JOIN `bbs_group` t2 ON t1.groupId = t2.id LEFT JOIN `bbs_dept` t3 ON t1.deptId = t3.id WHERE groupId='$groupId' ORDER BY itemLevel,itemId";
	}
            ...
            <thead>
              <tr>
                ...
                <td align="left">              	
                    ...
                    <input id="groupId" name="groupId" type="hidden" value="<?php if(isset($_GET['groupId'])){echo $_GET['groupId'];}?>">
